%%Heston Nandi Floating Lookback Option
...
%Generate Expected Payoff Paths
PayoffC = zeros(1,N);
for k = 1:N
PayoffC(1,k) = max(P(:,k));
end
...
PayoffC1 = PayoffC-0.949;
PayoffP1 = 0.949-PayoffP;
Payoff2C = zeros(1,N);
for k = 1:N
...
%Floating lookback call & put prices
FC=sum(Payoff2C)/N*exp(-0.00357/365*127);
FP=sum(Payoff2P)/N*exp(-0.00357/365*127);